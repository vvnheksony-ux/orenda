...
function LoginModalContent({
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!isValidEmail(email)) { setErrorMsg(t('validEmail')); return }
    if (!password) { setErrorMsg(t('enterPassword')); return }
    setLoading(true)
    setErrorMsg('')
    setShowResend(false)
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!isValidEmail(email)) { setErrorMsg(t('validEmail')); return }
    if (password.length < 8) { setErrorMsg(t('passwordMin')); return }
  ...
  const handleResend = async () => {
    ...
    if (error) setErrorMsg(friendlyError(error.message))
    else setNoticeMsg(t('confirmationSent', { email }))
    setLoading(false)
  }
  ...
  const title = view === 'login' ? t('signIn') : t('createAccount')
  const subtitle = view === 'login' ? t('signInSubtitle') : t('registerSubtitle')
          ...
          >
            ...
            {mode === 'options' && (
              <div className="flex flex-col gap-3">
                <button onClick={() => chooseMethod('email')} disabled={!!pendingMethod} type="button" className={`${optionBtnCls} disabled:opacity-60`}>
                  ...
                  {pendingMethod === 'email' ? t('loading') : view === 'login' ? t('continueEmail') : t('signUpEmail')}
                </button>
                ...
                <button onClick={() => chooseMethod('phone')} disabled={!!pendingMethod} type="button" className={`${optionBtnCls} disabled:opacity-60`}>
                  ...
                  {pendingMethod === 'phone' ? t('loading') : view === 'login' ? t('continuePhone') : t('signUpPhone')}
                </button>
                ...
                <button onClick={handleGoogle} disabled={!!pendingMethod} type="button" className="w-full py-3 px-4 rounded-[12px] border border-gold-200 font-dm-sans text-[15px] font-medium text-gold-900 flex items-center justify-center gap-3 hover:bg-white transition-colors bg-white disabled:opacity-60">
                  ...
                  {pendingMethod === 'google' ? t('connecting') : view === 'login' ? t('continueGoogle') : t('signUpGoogle')}
                </button>
              ...
              <form onSubmit={handleEmailLogin} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <div className="relative">
                    <input type={showLoginPassword ? 'text' : 'password'} placeholder={t('passwordPlaceholder')} value={password} onChange={e => setPassword(e.target.value)} required className={passwordInputCls} />
                    <button
                      type="button"
                      onClick={() => setShowLoginPassword(v => !v)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-gold-500 hover:text-gold-800 transition-colors"
                      aria-label={t(showLoginPassword ? 'hidePassword' : 'showPassword')}
                    >
            ...
            {mode === 'email' && view === 'register' && (
              <form onSubmit={handleEmailRegister} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[13px] font-semibold text-gold-900 font-dm-sans">{t('passwordLabel')}</label>
                  <div className="relative">
                    <input type={showRegisterPassword ? 'text' : 'password'} placeholder={t('passwordMinPlaceholder')} value={password} onChange={e => setPassword(e.target.value)} required className={passwordInputCls} />
                    <button
                      type="button"
                      onClick={() => setShowRegisterPassword(v => !v)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-gold-500 hover:text-gold-800 transition-colors"
                      aria-label={t(showRegisterPassword ? 'hidePassword' : 'showPassword')}
                    >
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[13px] font-semibold text-gold-900 font-dm-sans">{t('confirmPassword')}</label>
                  <div className="relative">
                    <input type={showConfirmPassword ? 'text' : 'password'} placeholder={t('repeatPassword')} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required className={passwordInputCls} />
                    <button
                      type="button"
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-[13px] font-semibold text-gold-900 font-dm-sans">{t('phoneOptional')}</label>
                  <input type="tel" placeholder={t('phonePlaceholder')} value={phoneSignup} onChange={e => setPhoneSignup(e.target.value)} className={inputCls} />
                </div>
                <button disabled={loading} type="submit" className={primaryBtnCls} style={{ background: '#b89148' }}>
                  {loading && <span className="inline-block w-4 h-4 border-2 border-white/60 border-t-transparent rounded-full animate-spin mr-2 align-[-2px]" />}
            ...
            {mode === 'phone' && phoneStep === 'phone' && (
              <form onSubmit={handleSendPhoneOtp} className="flex flex-col gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[13px] font-semibold text-gold-900 font-dm-sans">{t('phoneOptional')}</label>
                  <input type="tel" placeholder={t('phonePlaceholder')} value={phone} onChange={e => setPhone(e.target.value)} required className={inputCls} />
                </div>
                <button disabled={loading} type="submit" className={primaryBtnCls} style={{ background: '#b89148' }}>
                  {loading && <span className="inline-block w-4 h-4 border-2 border-white/60 border-t-transparent rounded-full animate-spin mr-2 align-[-2px]" />}
                  {loading ? t('sendingCode') : view === 'login' ? t('sendOtp') : t('sendVerification')}
                </button>
              </form>
            )}
            ...
            {mode === 'phone' && phoneStep === 'otp' && (
              <form onSubmit={handleVerifyPhoneOtp} className="flex flex-col gap-5">
                <div className="flex flex-col gap-1 text-center">
                  <p className="font-dm-sans text-[14px] text-gold-700">{t('sentTo', { target: phone })}</p>
                  <p className="font-dm-sans text-[13px] text-emerald-600">{t('autoDetecting')}</p>
                </div>
                ...
                <button disabled={loading || otp.replace(/\D/g, '').length < 6} type="submit" className={primaryBtnCls} style={{ background: '#b89148' }}>
                  {loading && <span className="inline-block w-4 h-4 border-2 border-white/60 border-t-transparent rounded-full animate-spin mr-2 align-[-2px]" />}
                  {loading ? t('verifying') : view === 'login' ? t('verifyAndSignIn') : t('verifyAndCreate')}
                </button>

                <p className="text-center font-dm-sans text-[13px] text-gold-600">
                  {resendIn > 0 ? (
                    <>{t('resendCodeIn', { time: `${Math.floor(resendIn / 60)}:${String(resendIn % 60).padStart(2, '0')}` })}</>
                  ) : (
