...
function LoginForm() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    if (error) { setErrorMsg(error.message) } else { window.location.href = nextUrl }
    setLoading(false)
  }
          ...
          {mode === 'email' && (
            <form onSubmit={handleEmailLogin} className="flex flex-col gap-3">
              ...
              <div className="text-right -mt-1">
                <Link href="/forgot-password" className="font-dm-sans text-[12px] text-gold-600 hover:text-gold-800 transition-colors">
                  Forgot password?
                </Link>
              </div>
