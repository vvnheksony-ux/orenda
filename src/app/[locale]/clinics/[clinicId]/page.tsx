...
export default function ClinicDetailPage({ params }: { params: Promise<{ clinicId: string }> }) {
  ...
  const [clinic, setClinic] = useState<Department | null>(null)
  const [others, setOthers] = useState<Department[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const isNumeric = /^\d+$/.test(clinicId)
    const param = isNumeric ? `id=${clinicId}` : `slug=${clinicId}`
    Promise.all([
      fetch(`/api/departments?locale=${locale}&${param}`).then(r => r.json()),
      fetch(`/api/departments?locale=${locale}&limit=20`).then(r => r.json()),
    ])
      .then(([single, all]) => {
        const found: Department | null = single && !single.error ? single : null
        setClinic(found)
        const others: Department[] = (all?.docs || []).filter((d: any) => d.icon && d.id !== found?.id).slice(0, 4)
        setOthers(others)
      })
      .catch(() => {})
        ...
        <div className="max-w-[1512px] mx-auto w-full flex flex-col gap-[80px] items-center pb-[120px] px-4 sm:px-8 lg:px-[80px] pt-[100px] lg:pt-[212px]">
          ...
          <div className="flex gap-[16px] h-[472px] items-center justify-center w-full">
            ...
            <div className="bg-white flex h-[472px] items-center justify-end overflow-hidden relative rounded-[16px] flex-1 max-w-[1352px]">
              <div className="relative h-[448px] rounded-[16px] shrink-0 w-[650px] overflow-hidden mr-[12px]">
                <Image src="/images/about/about-hero-3.jpg" alt={clinic.name} fill className="object-cover rounded-[16px]" sizes="650px" priority />
              </div>
              <div className="absolute left-[58px] top-[79px] flex flex-col gap-[40px] items-start" style={{ maxWidth: 'calc(100% - 720px)' }}>
                <h1 className="font-cormorant font-bold text-[48px] text-[#3b2d17] leading-none">{clinic.name}</h1>
                <div className="font-dm-sans font-light text-[24px] text-[#594522] leading-[1.4]">
                  <p className="mb-[12px]">World-class {clinic.name} services at Orienda International Hospital.</p>
          ...
          <div className="flex gap-[40px] items-center w-full max-w-[1352px]">
            {clinic.icon && (
              <div className="bg-white rounded-full overflow-hidden shrink-0 size-[160px]"
                ...
                <Image src={clinic.icon} alt={clinic.name} width={160} height={160} className="object-contain p-[16px]" unoptimized />
              </div>
            )}
            <div className="flex flex-col gap-[16px]">
              <h2 className="font-cormorant font-bold text-[48px] text-[#3b2d17] leading-none">{clinic.name}</h2>
              <p className="font-dm-sans text-[20px] text-[#594522]">
                Orienda International Hospital provides comprehensive {clinic.name} services with a dedicated team of specialists.
              </p>
            </div>
          </div>
          ...
          {others.length > 0 && (
            <div className="flex flex-col gap-[40px] items-center w-full max-w-[1352px]">
              <h3 className="font-cormorant font-bold text-[32px] text-[#3b2d17] leading-none">Other Clinics &amp; Departments</h3>
              <div className="flex gap-[40px] flex-wrap justify-center">
                {others.map(other => (
                  <Link key={other.id} href={`/clinics/${other.slug || other.id}` as any}
