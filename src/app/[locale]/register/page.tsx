...
export default function RegisterPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      window.location.href = '/' // Force full reload to update server cookies correctly
    }
    setLoading(false)
  }
        ...
        <div className="bg-white rounded-[24px] shadow-[0px_4px_32px_rgba(122,95,44,0.10)] p-8 flex flex-col gap-5">
          ...
          {usePhone ? (
            <div className="flex flex-col gap-4">
              <button type="button" onClick={() => { setUsePhone(false); setStep('phone'); setErrorMsg(''); }} className="self-start text-sm text-gold-600 font-medium hover:text-gold-800 transition-colors">
                ← Back to Options
              </button>
                ...
                <form onSubmit={handleSendOtp} className="flex flex-col gap-3">
                  <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      required
                      className="w-full px-4 py-3 rounded-[12px] border border-gold-200 outline-none focus:border-gold-500 font-dm-sans bg-gold-50/30"
                    />
                  </div>
                  <button disabled={loading} type="submit" className="w-full py-3 bg-[#6b5a45] hover:bg-[#5a4b39] transition-colors text-white rounded-[12px] font-bold font-dm-sans shadow-md mt-2 disabled:opacity-50">
                ...
                <form onSubmit={handleVerifyOtp} className="flex flex-col gap-3">
                   <div className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      required
                      className="w-full px-4 py-3 rounded-[12px] border border-gold-200 outline-none focus:border-gold-500 font-dm-sans bg-gold-50/30 text-center tracking-[0.5em] text-lg font-bold"
                    />
                  </div>
                  <button disabled={loading} type="submit" className="w-full py-3 bg-[#6b5a45] hover:bg-[#5a4b39] transition-colors text-white rounded-[12px] font-bold font-dm-sans shadow-md mt-2 disabled:opacity-50">
            ...
            <div className="flex flex-col gap-3">
              ...
              <button onClick={() => setUsePhone(true)} type="button" className="w-full py-3 px-4 rounded-[12px] bg-gold-50 border border-gold-100 font-dm-sans text-[15px] font-bold text-gold-900 flex items-center justify-center gap-3 hover:bg-gold-100 transition-colors shadow-sm">
